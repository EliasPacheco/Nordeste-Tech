...
func load_quiz() -> void:
	...
	var options = randomize_array(desafio.bd[index].options)
	...
	match desafio.bd[index].type:
		QuestionType.TEXT:
			$question_info/image_holder.hide()
			
		QuestionType.IMAGE:
			$question_info/image_holder.show()
			question_image.texture = desafio.bd[index].question_image
			
		QuestionType.VIDEO:
			$question_info/image_holder.show()
			question_video.stream = desafio.bd[index].question_video
			question_video.play()
		
		QuestionType.AUDIO:
			$question_info/image_holder.show()
			question_audio.stream = desafio.bd[index].question_audio
			question_audio.play()
